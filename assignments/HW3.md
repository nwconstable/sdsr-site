...
The feedback for our slides was fairly positive overall, but did point out a few things that I frankly agreed with, upon review. First, both reviews asked for more examples and/or imagery to help relate the ideas to real-world scenarios. To address this, we added an example of wildfire prediction and how that might be approach from two perspectives: homogeneous data vs hetereogeneous data (as well as accompanying imagery). Second, to relate *possible* applications to real-world applications, we added a slide about Relational Deep Learning, and RDLs connection to graph learning. We also added imagery for this. Finally we added an image showing a graph with hetereogeneous node types to, again, try to reinforce the difference between the two categories of data. The second commonality the reviews had was both asked for more specific details across the following topics: message passing, the explainer callback mechanics, and temporal graph sampling. To that end we added short bullet-points about the EdgeIndex tensor, PyG's temporal graph sampling, and the `Explainer` callback mechanism within the message passing process. 

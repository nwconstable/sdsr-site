...
We think the most unreasonable assumption the authors made is that anyone that reads their paper will be able to follow up with experimentating with their code.  Most of the applications the authors state PyG 2.0 is compatible with involve having computers that are able to hold and train with large sets of data in a reasonable time frame.  PyG also has to be compatible with the hardware available to the readers, and needing the NVIDIA RAPIDS (the framework for CuGraph intergration) can prevent users that cannot support NVIDIA applications on their computers from testing on their own. The authors also leave it to the readers to do their own research on how to construct LLMs or RDL apparatuses with PyG to confirm their conjectures in section 3.
